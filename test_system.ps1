...
try {
    ...
    Write-Host "OK Model Version: $($health.model_version)" -ForegroundColor Green
} catch {
...
Write-Host "  Testing NORMAL transaction..." -ForegroundColor Cyan
try {
    $result1 = Invoke-RestMethod -Uri "http://localhost:8000/predict" -Method Post -ContentType "application/json" -Body $normalTx
...
Write-Host "  Testing SUSPICIOUS transaction..." -ForegroundColor Cyan
try {
    $result2 = Invoke-RestMethod -Uri "http://localhost:8000/predict" -Method Post -ContentType "application/json" -Body $suspiciousTx
...
try {
    ...
    Write-Host "OK Total Predictions: $($metrics.total_predictions)" -ForegroundColor Green
    Write-Host "OK Frauds Detected: $($metrics.fraud_detected)" -ForegroundColor Green
    Write-Host "OK Fraud Rate: $([math]::Round($metrics.fraud_rate * 100, 2))%" -ForegroundColor Green
    Write-Host "OK Avg Response Time: $([math]::Round($metrics.avg_response_time * 1000, 2)) ms" -ForegroundColor Green
} catch {
    Write-Host "FAIL Metrics check failed" -ForegroundColor Red
}
Write-Host ""
...
if (Test-Path $logFile) {
    ...
    Write-Host "OK Predictions logged: $logCount entries" -ForegroundColor Green
} else {
